...
def get_energy(filename, exp_eng_peak, verbose=False):
    '''
    ...
    peak = get_peak(counts, exp_int_peak, debug=verbose)
...
if __name__ == "__main__":
    ...
    scatter_energies = [get_energy(f'goode/scatter{angle}.Chn', expected, verbose=False) for angle, expected in zip(angles, expected_scatter)]
    print("espected scattering energies:", expected_scatter)
    print("actual scattering energies:", scatter_energies)

    recoil_energies = [get_energy(f'goode/recoil{angle}.Chn', expected, verbose=False) for angle, expected in zip(angles, expected_recoil)]
