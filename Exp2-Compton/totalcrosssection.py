...
for i in range(len(raw_counts[0])):
    if (np.sqrt(raw_count_errors[0][i]**2 + raw_count_errors[-1][i]**2)) > 0: 
        ...
        difference.append(0)
fig, ax =plt.subplots()
#ax.plot(difference, label="Deviation")
# ax.plot([0,2048], [3,3], color='r',label="Cutoff")
# ax.plot([window_start, window_start], [-2,25], color='r')
# ax.plot([window_end, window_end], [-2,25], color='r')
ax.plot(raw_counts[0],label="No Attenuation",color='tab:blue')
ax.plot(raw_counts[2],color='tab:red', label="Medium Attenuation")
#for i in range(2, 5):
    #ax.plot(raw_counts[i],color='tab:red',alpha=0.5)
ax.plot(raw_counts[5], label="Max Attenuation",color='tab:orange')
...
plt.title("Count Rates and Selection Window")
#plt.plot([window_start, window_end],[0,0],label="window")

plt.legend()
plt.savefig('./images/raw_counts.png')
...
plt.errorbar(attenuation, [total_counts[i] for i in range(len(attenuation))],xerr=0.01,yerr=total_errors,ls="None",marker=".",label="Data Residuals")
...
plt.legend()
plt.title("Count Rates at Varying Attenuation")
plt.xlabel("Plastic Layer Thickness (cm)",size=12)
plt.ylabel("Count Rate (Hz)",size=12)
